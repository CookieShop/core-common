 ...
 * @author dev
 */
class Import implements \Countable, \Iterator{
    /**
     * @var SplFileObject
     */
    protected $file;
    ...
    /**
     * Create a CSV iterator. Will use the first record as 
    ...
    /**
     * Rewind the Iterator to the first element. Optionally consumes 
    ...
    public function rewind()
    {
            ...
            if ($this->useFirstRecordAsHeader)
            {
                if (!$this->file->valid())
                {
                    ...
                    'Expected first row to be header, but reached EOF instead');
                }
    ...
    /**
     * Return the row number of the current element. 
     * Note: does not start at zero.
     *
     * @return int
     */
    public function key()
    ...
    /**
     * Count number of records in file (excluding header if 
     * first record is used as header).
     *
     * @return int
     */
